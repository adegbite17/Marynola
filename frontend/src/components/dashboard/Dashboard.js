...
const Dashboard = () => {
    ...
    const handleDownloadId = async (staffId, staffName) => {
        try {
            ...
            setError('Failed to download ID proof, no ID for this staff. Please try again.');
        } finally {
        ...
        <Container maxWidth="lg">
            <Box sx={{mt: 4}}>
                ...
                <Typography variant="h5" gutterBottom sx={{mt: 4}}>
                    Staff
                    Members {safeFilteredStaff.length !== safeStaffList.length && `(${safeFilteredStaff.length} of ${safeStaffList.length})`}
                </Typography>
